...
with Path('data/dd-small-nasari-15.txt').open('r') as file:
...
stop_words = set(Path('data/Stop-Words.txt').open('r').read().splitlines())
...
with Path('data/Ebola-virus-disease.txt').open('r') as file:
    for i, line in enumerate(file.readlines()):
        if i == 3:
            title = line
